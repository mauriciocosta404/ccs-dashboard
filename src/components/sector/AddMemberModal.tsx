...
export default function AddMemberModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await httpClient.post(`/sectors/${sectorId}/membros`, {
        userId: selectedUserId,
      });

      toast.success("Membro adicionado ao sector com sucesso!");
      setSelectedUserId("");
      onClose();
      if (onSuccess) {
        onSuccess();
      ...
      const errorMessage =
        ...
        "Erro ao adicionar membro ao sector. Tente novamente.";
      ...
      <div className="no-scrollbar relative w-full overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
        <div className="px-2 pr-14">
          <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
            Adicionar Membro ao Sector
          </h4>
          <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
            Selecione um usuário para adicionar ao sector.
          </p>
        </div>
