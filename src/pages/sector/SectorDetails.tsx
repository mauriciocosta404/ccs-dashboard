...
import AddMemberModal from "../../components/sector/AddMemberModal";
...
export default function SectorDetails() {
  ...
  useEffect(() => {
    if (!id) {
      setError("ID do sector não encontrado na URL");
      setLoading(false);
      return;
    }
    ...
    const fetchSector = async () => {
      try {
        const response = await httpClient.get<Sector>(`/sector/${id}`);
  ...
  const handleRefreshSector = () => {
    if (!id) return;
    setLoading(true);
    httpClient.get<Sector>(`/sector/${id}`)
      .then((response) => {
        setSector(response.data);
      })
      .catch((err) => {
  ...
  const handleRemoveMember = async (userId: string) => {
    ...
    if (!confirm("Tem certeza que deseja remover este membro do sector?")) {
      return;
    }

    try {
      await httpClient.delete(`/sectors/${id}/membros/${userId}`);
      toast.success("Membro removido do sector com sucesso!");
      handleRefreshSector();
    } catch (error: unknown) {
      const errorMessage =
        (error as { response?: { data?: { message?: string } }; message?: string })?.response?.data?.message ||
        (error as { message?: string })?.message ||
        "Erro ao remover membro do sector. Tente novamente.";
  ...
  if (loading) {
    return (
      <div>
        <PageMeta
          title="Sector - ADONAI CCS"
          description="Detalhes do sector"
        />
    ...
    return (
      <div>
        <PageMeta
          ...
          description="Erro ao carregar sector"
        />
        ...
        <div className="min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/[0.03] xl:px-10 xl:py-12">
          <div className="mx-auto w-full max-w-4xl">
            <Alert
              variant="error"
              title="Erro"
              message={error || "Sector não encontrado"}
            />
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      <PageMeta
        title={`${sector.name} - ADONAI CCS`}
        description={`Detalhes do sector ${sector.name}`}
      />
      <PageBreadcrumb pageTitle={sector.name} />
      <div className="min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/[0.03] xl:px-10 xl:py-12">
        <div className="mx-auto w-full">
          ...
          <ComponentCard title="Membros do Sector">
                      ...
                      {loading ? (
                        ...
                        <TableRow>
                          <TableCell className="px-5 py-4 text-center text-gray-500 dark:text-gray-400">
                            Nenhum membro adicionado a este sector ainda.
                          </TableCell>
